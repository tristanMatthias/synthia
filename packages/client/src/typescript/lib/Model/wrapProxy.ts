...
export type ProxyExtended<T> = T & {
  [isProxy]: true;
  __update(prop: keyof T): void;
  __callbacks: ((v: any, p: keyof T) => any)[]
...
export const wrapProxy = <T extends object>(
  ...
  const handler: ProxyHandler<T> = {
    ...
    set<K extends keyof T>(target: ProxyExtended<T>, key: K, value: ProxyExtended<T>[K]) {
      ...
      target[key] = value;
      if (!ignoreProps.includes(key as string)) target.__update(key);
      return true;
    }
  };
  ...
  if (!(value as ProxyExtended<T>)[isProxy]) {
    ...
    root.__callbacks = [];
    root.__update = (prop: keyof T) => {
      root.__callbacks.forEach(cb => cb(root, prop));
